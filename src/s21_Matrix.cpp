...
S21Matrix S21Matrix::CalcComplements() const {
  ...
  if (rows_ == 1) throw std::logic_error("Invalid size of matrix");
  double sign = 1;
  ...
  for (int i = 0; i < rows_; i++) {
    for (int j = 0; j < cols_; j++) {
      ...
      result.matrix_[i][j] = (matrix_determinant_value == 0.0f ? 1 : sign) *
                             matrix_determinant_value;
    }
  }
  return result;
}
...
double S21Matrix::matrix_determinant(S21Matrix& result) const {
  ...
  if (matrix_order == 1) {
    ...
    for (int i = 0; i < result.cols_; i++) {
      ...
      return_value +=
          sign * result.matrix_[0][i] * matrix_determinant(new_matrix);
      sign = -sign;
    }
  }
  return return_value;
...
S21Matrix S21Matrix::InverseMatrix() const {
  ...
  if (fabs(determinant) < 1e-6) throw std::logic_error("Div by zero");
...
void S21Matrix::set_cols(int cols) {
  if (cols < 0) throw std::logic_error("Invalid size");
