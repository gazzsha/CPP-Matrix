...
TEST(Test, DefaultConstructor) {
  ...
  //  ASSERT_THROW(matrix(0, 0), std::invalid_argument);
}
...
TEST(Test, setRows) {
  S21Matrix matrix(2, 2);
  matrix.setter_rows(5);
  EXPECT_TRUE(matrix.get_rows() == 5);
}

TEST(Test, setCols) {
  S21Matrix matrix(2, 2);
  matrix.setter_cols(9);
...
TEST(Test, operator_left_right_bracket_error) {
  S21Matrix mat(1, 2);
  EXPECT_THROW(mat(-1, 3), std::logic_error);
}

TEST(Test, inverseerror_div) {
  S21Matrix mat(1, 1);
  mat(0, 0) = 0;
  EXPECT_THROW(mat.InverseMatrix(), std::logic_error);
}

TEST(Test, inverseerror_ones) {
  S21Matrix mat(1, 1);
  mat(0, 0) = 5;
  S21Matrix res = mat.InverseMatrix();
  EXPECT_DOUBLE_EQ(res(0, 0), 0.2);
}
